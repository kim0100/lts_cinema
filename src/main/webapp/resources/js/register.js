...
var id = document.querySelector('#cust_id');
...
var userName = document.querySelector('#cust_name');
...
function checkId() {
    var idPattern = /[a-zA-Z0-9]{8,15}/;
    if(id.value === "") {
        error[0].innerHTML = "아이디를 적어주세요.";
        error[0].style.display = "block";
    } else if(!idPattern.test(id.value)) {
        error[0].innerHTML = "8~15자의 영문 대 소문자, 숫자만 사용 가능합니다.";
        error[0].style.color = "red";
        error[0].style.display = "block";
    } else {
        error[0].innerHTML = "올바른 아이디 입니다.";
...
function checkPw() {
    ...
    if(pw1.value === "") {
        error[1].innerHTML = "비밀번호를 적어주세요.";
        error[1].style.display = "block";
    } else if(!pwPattern.test(pw1.value)) {
        error[1].innerHTML = "8~15자 영문 대 소문자, 숫자, 특수문자를 사용하세요.";
...
function isBirthCompleted() {
    ...
    if(!yearPattern.test(yy.value)) {
        error[4].innerHTML = "태어난 년도 8자리를 정확하게 입력하세요.";
...
function checkEmail() {
    var isEmail = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/i;
     ...
     if(!isEmail.test(email.value)) {
        error[6].innerHTML = "잘못된 이메일 형식입니다.";
